class Solution {
    public int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        ...
        int pigStates = totalTests + 1;
        
        // If one pig has 2 state it can check two buckets. If drank from bucket 1 and alive after time to die
        // then poison in bucket 2. If dead poison in bucket 1.
        
        // If two pigs has 2 states, pig1 drank from bucket1 and bucket2. pig2 drank from bucket2 and bucket3
        // If after the test, pig1 died poison in bucket1. pig2 died poison in bucket3. If both died poison in
        // bucket2. If none died poison in bucket4.
        
        // So number of buckets x pigs can test with s states will be (s^x).
        // We need x pigs such that (s^x) >= buckets.
        // x >= log(buckets)/log(s)
        
        return (int)Math.ceil(Math.log(buckets)/Math.log(pigStates));
    }
}