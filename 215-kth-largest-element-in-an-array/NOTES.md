...
​
* Partion method algo:
1. Store the value of pivot in a variable and have a index which will store the smaller values the pivot.
2. swap pivot index with right.
3. from left to right, if nums[i]<val then swap i and idx then increment idx.
4. once done swap idx with right since we have stored pivot there in the beginning return idx.
​
#### Complexity Analysis
